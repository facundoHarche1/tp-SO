...
t_peticion_fs recibirPeticionEscribir(int socket, t_log* logger)
{
	...
	log_info(logger, "Llego el PID: %d", aux.PID);
	log_info(logger, "Llego la direccion fisica: %d", aux.direcLogica);
	log_info(logger, "Llego el registro puntero: %d", aux.regPuntero);
	//retorno la estructura creada
	return aux;
}
